...
async function seedDebtTransactions() {
  console.log('Seeding 150 debt transactions...');
  ...
  for (let i = 0; i < 150; i++) {
    ...
    if ((i + 1) % 50 === 0) {
      console.log(`  ${i + 1}/150 debt transactions created`);
    }
  }
...
async function seedSalesTransactions() {
  console.log('Seeding 100 sales transactions...');
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if ((i + 1) % 25 === 0) {
      console.log(`  ${i + 1}/100 sales transactions created`);
    }
  }
...
async function seedLeaveTransactions() {
  console.log('Seeding 30 leave transactions...');
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if ((i + 1) % 10 === 0) {
      console.log(`  ${i + 1}/30 leave transactions created`);
    }
  }
...
async function seedBirthdays() {
  ...
  for (const birthday of birthdays) {
    ...
    const existing = await pool.query(
      'SELECT id FROM birthdays WHERE name = $1',
      [birthday.name]
    );
    ...
    if (existing.rows.length > 0) {
      // Update existing
      await pool.query(
        'UPDATE birthdays SET date_of_birth = $1 WHERE name = $2',
...
async function main() {
  console.log('🚀 Starting reasonable test data seeding...\n');
  ...
  try {
    ...
    console.log('\nSummary:');
    console.log('  - 150 debt transactions (will show 100, then "Load More" for 50 more)');
    console.log('  - 100 sales transactions');
    console.log('  - 30 leave entries');
    console.log('  - 6 birthdays');
    console.log('\n💡 This is a reasonable amount to test pagination without lag!');
